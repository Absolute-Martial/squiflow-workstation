...
bool take_activation_request() noexcept override{if(state_!=InstanceState::Primary)return false;const int a=::open(activation_path_.c_str(),O_RDWR|O_CLOEXEC|O_NOFOLLOW);if(a<0)return false;char byte{};const bool present=::read(a,&byte,1)==1;if(present){const int truncated=::ftruncate(a,0);(void)truncated;}::close(a);return present;}
