...
class PagedListCache final{public:static constexpr std::size_t kMaximumPages=3,kMaximumPageRows=100;std::uint64_t begin_query();bool apply(std::uint64_t,std::vector<RowInput>);void select(std::string_view);std::size_t row_count()const noexcept;std::size_t page_count()const noexcept{return pages_.size();}std::optional<std::string> selected()const{return selected_;}private:bool contains(std::string_view)const;std::uint64_t generation_{0};std::deque<std::unique_ptr<PageBuffer>> pages_;std::optional<std::string> selected_;};
}
